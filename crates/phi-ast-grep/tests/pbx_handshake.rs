...
/// 扩展名（Hello 里自报的名字）。
const EXTENSION_NAME: &str = "phi-ast-grep";

/// 期望注册的斜杠命令。
const EXPECTED_COMMANDS: &[&str] = &["ast-grep"];

/// 期望注册的 LLM 工具。
const EXPECTED_TOOLS: &[&str] = &["ast_grep_search", "ast_grep_replace"];
...
fn extension_should_complete_pbx_lifecycle() {
    ...
    // 4) 宿主发 Shutdown，扩展回 ShutdownAck 后退出。
